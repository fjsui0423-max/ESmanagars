...
struct ESQuestionEditView: View {
    ...
    init(question: ESQuestion, onSave: @escaping (String, Int16) -> Void) {
        self.onSave = onSave
        _questionText  = State(initialValue: question.questionText ?? "")
        let max = question.maxLength
        _maxLengthText = State(initialValue: max > 0 ? "\(max)" : "")
    }
