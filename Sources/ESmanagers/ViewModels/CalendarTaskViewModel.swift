...
final class CalendarTaskViewModel: ObservableObject {
    ...
    @Published var filteredAptitudeTests: [AptitudeTest] = []
    ...
    func fetch() {
        ...
        let aptReq = AptitudeTest.fetchRequest()
        aptReq.predicate = NSPredicate(format: "deadlineAt != nil")
        aptReq.sortDescriptors = [NSSortDescriptor(keyPath: \AptitudeTest.deadlineAt, ascending: true)]
        allAptitudeTests = (try? context.fetch(aptReq)) ?? []
    ...
    var isEmpty: Bool {
        filteredBoxes.isEmpty && filteredAptitudeTests.isEmpty && filteredInterviews.isEmpty
    }
    ...
    func boxes(for date: Date) -> [ESBox] {
        allBoxes.filter {
            guard let d = $0.deadlineAt else { return false }
            return cal.isDate(d, inSameDayAs: date)
        }
    }

    func aptitudeTests(for date: Date) -> [AptitudeTest] {
        allAptitudeTests.filter {
            guard let d = $0.deadlineAt else { return false }
            return cal.isDate(d, inSameDayAs: date)
        }
    }
    ...
    func interviews(for date: Date) -> [Interview] {
        allInterviews.filter {
            guard let d = $0.startAt else { return false }
            return cal.isDate(d, inSameDayAs: date)
        }
    }
    ...
    private func filterItems() {
        filteredBoxes = allBoxes.filter {
            guard let d = $0.deadlineAt else { return false }
            return cal.isDate(d, inSameDayAs: selectedDate)
        }
        filteredAptitudeTests = allAptitudeTests.filter {
            guard let d = $0.deadlineAt else { return false }
            return cal.isDate(d, inSameDayAs: selectedDate)
        }
